...
export default function VisitorTrackingReportComponent({ height = 80 }) {
  const fetchExhibitors = async ({ page = 1, pageSize = 10 }) => {
  ...
  const { isLoading, data, isError, error, fetchNextPage } = useQuery({
    ...
    queryFn: ({ pageParam }) => fetchExhibitors({ page: pageParam ?? 1 }),
    ...
    <div>
      ...
      <div className={`ag-theme-alpine h-[${height}vh] w-full`}>
        <AgGridReact
          ...
          columnDefs={visitorsTrackingDef} // Column Defs for Columns
